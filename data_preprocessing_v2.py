...
# ============================================================================
# 【图像尺寸修改位置】
# 只需修改下面这个数字，即可改变整个脚本默认输出的正方形尺寸。
# 例如：256x256 -> 256，512x512 -> 512，1024x1024 -> 1024。
# 也可以不改代码，在运行命令末尾添加：--size 256
# ============================================================================
DEFAULT_SIZE = 512
...
def audit_task1_dataset(
    ...
    for source_image_path in tqdm(image_paths, desc="Audit Task 1", unit="image"):
...
def audit_task2_dataset(
    ...
    """Audit five Task 2 masks and write a per-image feature-presence table."""
    images_dir = data_root / "images"
    ...
    rows: list[dict[str, Any]] = []
    image_paths = sorted(images_dir.glob("*.jpg"))
    for source_image_path in tqdm(image_paths, desc="Audit Task 2", unit="image"):
        image_id = source_image_path.stem
        flags: list[str] = []
        width = 0
        height = 0
    ...
    # Compact table requested for modelling/reporting: one row per source
    # image and one 0/1 column per Task 2 feature. A value of 1 means that the
    # corresponding ground-truth mask contains at least one positive pixel.
    feature_table_path = output_path.with_name(
        f"{output_path.stem}_feature_presence.csv"
    )
    feature_rows = []
    for row in rows:
        ...
        feature_row.update(
            {
                attribute: row[f"{attribute}_present"]
                for attribute in TASK2_ATTRIBUTES
            }
        )
        feature_row["feature_count"] = sum(
            int(feature_row[attribute]) for attribute in TASK2_ATTRIBUTES
        )
        feature_row["status"] = row["status"]
        feature_rows.append(feature_row)

    with feature_table_path.open("w", newline="", encoding="utf-8-sig") as file:
    ...
    print_processed_lesion_average(rows, size)
    print(f"Report: {output_path.resolve()}")
    print(f"Feature presence table: {feature_table_path.resolve()}")
...
def build_transforms(size: int = DEFAULT_SIZE, seed: int = 42):
    """Return mild random training and deterministic validation transforms.

    Training uses only the four requested augmentation families: horizontal
    flip, vertical flip, mild affine, and mild brightness/contrast. The
    unmodified base version is represented by the validation transform.
    """
...
def _fixed_task1_augmentation_transforms(size: int, seed: int):
    """Return the five requested deterministic 512-ready data variants.

    Unlike ``build_transforms`` (which samples a new random augmentation at
        training time), these transforms write a reproducible set containing base,
        horizontal flip, vertical flip, mild affine, and mild brightness/contrast.
    """
    A, cv2, _ = _augmentation_dependencies()
    preprocess = [
        A.LongestMaxSize(
...
def prepare_fixed_task1_training_set(
    ...
    created first; each training ID receives exactly five versions: base,
    horizontal flip, vertical flip, mild affine, and mild brightness/contrast.
    Validation receives only deterministic base preprocessing.
    """
    ...
    for split_name, manifest, image_dir, mask_dir in (
        ...
        written_samples = 0
        progress = tqdm(manifest_rows, desc=f"Prepare Task 1 {split_name}", unit="image")
        for row in progress:
            image_id = row["image_id"]
            image, mask = read_pair(data_root, image_id)
            ...
            for variant, transform in selected_transforms.items():
                ...
                written_samples += 1
            progress.set_postfix(samples=written_samples)
...
def prepare_fixed_task2_training_set(
    ...
    validation_transform = build_transforms(size=size, seed=seed)[1]
    audit_rows_by_id = {
        row["image_id"]: row for row in _read_csv_rows(audit_path)
    }
    output_root = output_root.resolve()
    train_images = output_root / "train" / "images"
    ...
    for split_name, manifest, image_dir, mask_dir in (
        ...
        manifest_rows = _read_csv_rows(manifest)
        written_samples = 0
        progress = tqdm(manifest_rows, desc=f"Prepare Task 2 {split_name}", unit="image")
        for row in progress:
            image_id = row["image_id"]
            image, masks = read_task2_pair(data_root, image_id)
            selected_transforms = transforms if split_name == "train" else {"base": validation_transform}
                ...
                rows.append(
                    {
                        ...
                        "image_path": str(image_target.relative_to(output_root)),
                        **{
                            attribute: audit_rows_by_id[image_id][
...
def parse_args() -> argparse.Namespace:
    ...
    prepare_parser = subparsers.add_parser(
        ...
        help="write five-version 512x512 Task 1/2 train sets and clean val sets",
    )
    ...
    # 默认读取脚本顶部标注的 DEFAULT_SIZE；命令行 --size 可临时覆盖它。
    prepare_parser.add_argument("--size", type=int, default=DEFAULT_SIZE)
