...
def load_model(checkpoint_path: Path, device: torch.device) -> nn.Module:
    ...
    if hasattr(model, "normalize_input"):
        model.normalize_input = bool(checkpoint.get("pretrained_encoder", False))
...
def main() -> None:
    ...
    parser.add_argument("--ground-truth", type=Path, default=settings.inference_ground_truth, help="Optional Task 1 GT-mask folder, used only for visual comparisons.")
